...
def getCompaniesByRegion(region, page=1, activity=None):
    ...
    if region:
        if activity:
            ...
            dict = Companies.query.filter(Companies.activity.like(activitysearch),
                                          Companies.region == region).paginate(page, POSTS_PER_PAGE,
                                                                               False).items
            dictSchema = CompanyClientSchema(many=True)
            c = db.session.query(Companies.id).filter(Companies.activity.like(activitysearch),
                                                      Companies.region == region).count()
        else:
            dict = Companies.query.filter_by(region=region).paginate(page, POSTS_PER_PAGE,
                                                                     False).items
            dictSchema = CompanyClientSchema(many=True)
            c = db.session.query(Companies.id).filter_by(region=region).count()
        return jsonify(
            {"companies": dictSchema.dump(dict), "activity": activity, "name": region.text,
...
def getSearchCompanies(search, region=0, page=1, activity=None):
    ...
    in_company = []
    cw = db.session.query(CompaniesWaste.itn).with_entities(CompaniesWaste.itn).filter_by(
        fkko_id=search.replace(' ', '')).group_by(
    ...
    if len(fkkolist) > 0:
        cw = db.session.query(CompaniesWaste.itn).with_entities(CompaniesWaste.itn).filter(
            CompaniesWaste.fkko_id.in_(fkkolist)).group_by(
            CompaniesWaste.itn).all()
        for row in cw:
            in_company.append(row.itn)
    ...
    if region == 0:
        ...
        if activity:
            ...
            c = db.session.query(Companies.id).filter(
                or_(and_(Companies.activity.like(activitysearch), Companies.name.like(likesearch)),
                    and_(Companies.activity.like(activitysearch),
                         Companies.itn.in_(in_company)))).count()
            dict = Companies.query.filter(
                or_(and_(Companies.activity.like(activitysearch), Companies.name.like(likesearch)),
                    and_(Companies.activity.like(activitysearch),
                         Companies.itn.in_(in_company)))).paginate(page, POSTS_PER_PAGE,
                                                                   False).items
        else:
            c = db.session.query(Companies.id).filter(
                or_(Companies.name.like(likesearch), Companies.itn.in_(in_company))).count()
            dict = Companies.query.filter(
                or_(Companies.name.like(likesearch), Companies.itn.in_(in_company))).paginate(page,
                                                                                              POSTS_PER_PAGE,
                                                                                              False).items
    else:
        ...
        if activity:
            activitysearch = "%{}%".format(activity)
            c = db.session.query(Companies.id).filter(and_(Companies.activity.like(activitysearch),
                                                           or_(Companies.name.like(likesearch),
                                                               Companies.itn.in_(in_company)),
                                                           Companies.region_id == region)).count()
            dict = Companies.query.filter(and_(Companies.activity.like(activitysearch),
                                               or_(Companies.name.like(likesearch),
                                                   Companies.itn.in_(in_company)),
                                               Companies.region_id == region)).paginate(page,
                                                                                        POSTS_PER_PAGE,
                                                                                        False).items
        else:
            c = db.session.query(Companies.id).filter(
                and_(or_(Companies.name.like(likesearch), Companies.itn.in_(in_company)),
                     Companies.region_id == region)).count()
            dict = Companies.query.filter(
                and_(or_(Companies.name.like(likesearch), Companies.itn.in_(in_company)),
                     Companies.region_id == region)).paginate(page, POSTS_PER_PAGE, False).items

    dictSchema = CompanyClientSchema(many=True)
    return jsonify({"companies": dictSchema.dump(dict), "activity": activity, "region": region_name,
                    "count": "{}".format(math.ceil(c / POSTS_PER_PAGE))}), 200
