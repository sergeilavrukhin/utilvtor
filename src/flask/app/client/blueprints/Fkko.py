...
def getFkkoById(fkko_id):
    ...
    breadcrumb.reverse()
    return jsonify({"breadcrumb": breadcrumb, "fkko": fkkoSchema.dump(fkko),
                    "codes": codesSchema.dump(codes)}), 200


@app.route("/search/<string:fkko>")
def searchFkkoById(fkko):
    codes = Fkko.query.filter(
        and_(or_(Fkko.id.like("%{}%".format(fkko)), Fkko.name.like("%{}%".format(fkko))),
             Fkko.fkkoclass_id.isnot(None))).limit(10).all()
    codesSchema = FkkoClientSchema(many=True, only=("id", "name", "codespace"))
    return jsonify(codesSchema.dump(codes)), 200
