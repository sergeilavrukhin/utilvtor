    ...
    <div class="row">
      <div class="col-md-8">
        <b-card v-for="(item, index) in regions" :key="index" class="mt-3 mb-3">
          <b-card-text>
            <a :href="`companies/region/${item.code}`"><h2>{{item.text}}</h2></a>
            ...
            <ul class="activity" v-if="item.activity">
              <li v-for="(c_item, c_index) in item.activity" :key="c_index">
                <a :href="`companies/region/${item.code}/activity/${c_index}`">{{getActivity(activities, c_index)}}: {{c_item}}</a>
              </li>
            </ul>
...
export default {
  async asyncData({ $axios }) {
    const regions = await $axios.$get('dicts/regions').then((response) => {
      return response;
    }).catch((error) => {
      console.log(error);
    });
